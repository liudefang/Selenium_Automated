...
import cn.erp.keywords.configuration.ProductfeeKeyWords;
 ...
 */
public class TestProductfeeByExcel {
    public static Method method[];
    public static String keyword;
    public static String value;
    public static ProductfeeKeyWords keyWordsaction;
    ...
    public void testTestSuite() throws Exception{
        //声明一个关键动作类的实例
        keyWordsaction = new ProductfeeKeyWords();
        ...
        //读取“测试用例集合”Sheet中每行的是否运行列中的值
         ...
         testStep = ExcelUtil.getFirstRowContainsTestCaseID( Constants.Sheet_TestSteps1,testCaseID,Constants.Col_TestCaseID );
         //在“提交合同”Sheet中，获取当前要执行测试用例的最后一个步骤所在行行号
             testLastStep = ExcelUtil.getTestCaseLastStepRow( Constants.Sheet_TestSteps1,testCaseID,testStep );
             //遍历测试用例中的所有测试步骤
             for(;testStep < testLastStep;testStep++) {
              //从“提交合同”Sheet中读取关键字和操作值，调用execute_Action方法
                 keyword = ExcelUtil.getCellData( Constants.Sheet_TestSteps1,testStep,Constants.Col_KeyWordAction );
                 //在日志文件中打印关键字信息
                 Log.info("从Excel文件读取到的关键字是:"+keyword);

                 value = ExcelUtil.getCellData( Constants.Sheet_TestSteps1,testStep,Constants.Col_ActionValue);
                ...
                *作为测试方法的函数值进行调用
                ...
                if(method[i].getName().equals( keyword )) {
                    ...
                    if (testResult == true) {
                         ...
                         */
                        ExcelUtil.setCellData( Constants.Sheet_TestSteps1,testStep,Constants.Col_TestStepTestResult,"测试步骤执行成功" );
                        break;
                    } else {
                        /*当前测试步骤执行失败，在“提交合同”Sheet中，会将当前执行的测试步骤结果设定为“测试执行失败”

                         */
                        ExcelUtil.setCellData( Constants.Sheet_TestSteps1,testStep,Constants.Col_TestStepTestResult,"测试步骤执行失败" );
                        //测试步骤执行失败，则直接关闭浏览器，不再执行后续的测试步骤
                       ProductfeeKeyWords.close_browser( "" );
                        break;
                    }
